...
def index():
    ...
    if request.method == "POST":
        ...
        if status == "In":
            ...
            in_time = record.iloc[0]["In Time"]
            in_date = record.iloc[0]["Date"]

            in_dt = datetime.strptime(f"{in_date} {in_time}", "%Y-%m-%d %H:%M:%S")
            delta = now - in_dt
...
def download_report():
    ...
    if not in_df.empty:
        in_df["Out Time"] = ""
        in_df["Total Time"] = ""
        in_df["Status Detail"] = "In Progress"

    if not done_df.empty:
        done_df["Status Detail"] = "Completed"

    final_df = pd.concat([in_df, done_df], ignore_index=True)
    ...
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        ...
        row1 = workbook.add_format({"bg_color": "#F3F4F6"})
        row2 = workbook.add_format({"bg_color": "#FFFFFF"})

        for i in range(1, len(final_df) + 1):
            worksheet.set_row(i, None, row1 if i % 2 == 0 else row2)

    output.seek(0)
    return send_file(output,
                     download_name="VAS_Report.xlsx",
                     as_attachment=True,
                     mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
